...
var casualImgs = document.querySelector(".puma-casuals-imgs");
...
function onInIt(ArrProductsNew){
    ...
    casualImgs.innerHTML = '';

    ArrProductsNew.forEach((item,key)=>{
        let div = document.createElement("div");
        div.classList.add("puma-casuals-img");

      div.innerHTML = ` <img src="./images/pumaCasualShoes/pumaCasualShoe${item.id}.avif"
            class="puma-casuals-img-${item.id}"/>
            <div class="info-whist-cart">
                <div class="puma-casuals-info">
                    <h3>${item.brand}</h3>
                    <p>${item.name}</p>
                    <h5>Rs. ${item.price}</h5>
                </div>
...
function reloadCart(){
    ...
    checkOutList.forEach((item,key)=>{
        ...
        li.innerHTML = `
        <img src="./images/pumaCasualShoes/pumaCasualShoe${item.id}.avif">
        <div>${item.name}</div>
        <div>${item.price}</div>
        <div>
...
var pumaCasuals = document.querySelector('.puma-casuals');
// var search = document.querySelector('.search-bar-input');
var productList = pumaCasuals.querySelectorAll('.puma-casuals-img');
// var casualImgs = document.querySelector('.puma-casuals-imgs');
let enteredValue = "";
...
//         }
//         casualImgs.style.justifyContent = 'space-around';
//     }
//    else{
...
//         else{
//             productList[count].style.display = 'block';
//             casualImgs.style.justifyContent = 'flex-start';
//         }
//     }
//    }
